...
{
	uint count = 0;
	uint start = (uint)(GetCurrentTimeSeconds()*.01);
	do {
		...
		++count;
	} while (GetCurrentTimeSeconds()*.01 - (double)start < ms);
...
void JobSystemStartup(uint job_category_count, int generic_thread_count /*= -1*/)
{
	...
	gJobSystem->queues = new Thread::Queue<Job*>[job_category_count];
	...
	Thread::Create(GenericJobThread, gJobSystem->signals[JOB_GENERIC]);
